...
Result move(int x, int y, Symbol symbol)
{
	if (x < 3 && x >= 0 && y < 3 && y >= 0 && grid[y][x] == Symbol::EMPTY)
	{
		...
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
				if (grid[i][j] == Symbol::CIRCLE || grid[i][j] == Symbol::CROSS) numTurns++;
		...
		else if (x > 3 && x <= 0 && y > 3 && y <= 0 || grid[y][x] != Symbol::EMPTY) {
			return Result::WRONG;
		}
	}
}
	...
	void print()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				if (grid[i][j] == Symbol::CIRCLE)
					std::cout << 'O' << "|";
				else if (grid[i][j] == Symbol::CROSS)
					std::cout << 'X' << "|";
				else if (grid[i][j] == Symbol::EMPTY)
					std::cout << ' ' << "|";
			}
			std::cout << "\n";
		}
